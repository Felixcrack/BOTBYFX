...
/******** OWNER NUMBER**********/
const ownerNumber = ["12603763944@s.whatsapp.net","5213317595519@s.whatsapp.net"] 
/************************************/
           ...
           return false
			...
			switch(command) { 
                ...
                case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}glitchtext Bot & Fx`)
					reply(ind.wait())
				...
				case 'tahta':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}hartatahta Fx`)
					har = body.slice(12)
					reply('「❗」Hartatahta')
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
                ...
                addKoinUser('12603763944@s.whatsapp.net', fee)
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Wenas ahora tengo vida propia')
					if (Number(args[0]) === 1) {
				...
				case 'setppbot':
					...
					reply('Gracias por el nuevo perfil te amo😗')
					break 
				case 'setpp': 
                        if (!isGroup) return reply(ind.groupo())
                       if (!isGroupAdmins) return reply(ind.admin())
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  reply(`Hola *${pushname}*, Ese comando *${prefix}${command}* No esta en mi menu, usa *${prefix}menu* para verlo`)
                  }
            if (/^>/.test(pes)) {
	            let txt = pes.replace(/^>/, '')
	            let type = Function
